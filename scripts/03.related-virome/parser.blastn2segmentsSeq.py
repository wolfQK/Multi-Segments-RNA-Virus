...
parser.add_argument('--blastn_out_file', type=str, default=None)
...
nucl_accession2taxonomy_db = '/hwfsxx1/ST_HN/P20Z10200N0206/USER/fengqikai/biye/Multi-Segments-Virus/data/RdRp/db_sqlite3/virus.nucl.accession2taxonomy.db'
## blastn结果
blastn_out_file = args.blastn_out_file
## 相差范围 
...
# 1 信息读取
## 1.1 tblastx结果
names = ['qseqid', 'qlen', 'sseqid', 'slen', 'length', 'pident', 'mismatch', 'gapopen', 'gaps', 'qcovs', 'qstart', 'qend', 'sstart', 'send', 'evalue', 'bitscore']
blastn_out = pd.read_csv(blastn_out_file, sep='\t', names=names)
blastn_out.shape
...
def get_useful_blast_info(blast_out=None, related_p=100):
    out_list = []
    for contig_id in blast_out['qseqid'].drop_duplicates():
        tmp_df = blast_out.query('qseqid == @contig_id')
...
## 3.0 比对结果预处理
### 3.0.1 获取有效的比对结果
blastn_use = get_useful_blast_info(blast_out=blastn_out, related_p=80)
### 3.0.2 获取比对上的contigs-segments
contigs_segments = blastn_use[['qseqid', 'sseqid']].drop_duplicates() # 只考虑是否有效比对，不考虑比对上了哪部分
...
conn = sqlite3.connect(nucl_accession2taxonomy_db)
cur = conn.cursor()
# 获取比对上的segments的分类信息
# 也就是该segments潜在的分类信息
nucl_taxonomy_list = []
...
for contig_id, segment_id in contigs_segments_list:
    ...
    try :
        ...
        nucl_taxonomy_list.append(found)
    except:
    ...
    nucl_taxonomy_df = pd.DataFrame(data=nucl_taxonomy_list, columns=header)

# 关闭游标和连接
cur.close()
conn.close()

nucl_taxonomy_df.fillna('Unclassified', inplace=True)

### 3.0.4 整合比对信息和分类信息
contigs_segments_blastn_taxonomy = pd.merge(blastn_use, nucl_taxonomy_df, how='left', left_on=['qseqid', 'sseqid'], right_on=['contig_id','Accession'])

### 3.0.5 找比对上的基因组
# 以taxid来区分基因组; 还没有验证这是否可行
Taxid_list = list(contigs_segments_blastn_taxonomy['Taxid'].drop_duplicates())

## 3.1 选比对上的基因组的每条节段的最优比对
### 3.1.1 找潜在的基因组
found_Virome = dict()
for taxid in Taxid_list:
    tmp = contigs_segments_blastn_taxonomy[contigs_segments_blastn_taxonomy['Taxid'] == taxid] # 该基因组的全部比对结果
...
##### 统计比对上的基因组的分类信息
contig_uniq = nucl_taxonomy_df['contig_id'].drop_duplicates()
Taxid_uniq = nucl_taxonomy_df['Taxid'].drop_duplicates()
Order_uniq = nucl_taxonomy_df['Order'].drop_duplicates().dropna()
Family_uniq = nucl_taxonomy_df['Family'].drop_duplicates().dropna()
Genus_uniq = nucl_taxonomy_df['Genus'].drop_duplicates().dropna()
virusName_uniq = nucl_taxonomy_df['Virus_name'].drop_duplicates()
...
for taxid in Taxid_uniq:
    tmp = nucl_taxonomy_df[nucl_taxonomy_df['Taxid'] == taxid]
...
for genus in Genus_uniq:
    tmp = nucl_taxonomy_df[nucl_taxonomy_df['Genus'] == genus]
    seq_id = list(tmp['contig_id'].drop_duplicates())
    seq_info = tmp[['Taxid', 'contig_id', 'Accession', 'Family', 'Order', 'Genus', 'Virus_name']].drop_duplicates()
...
for family in Family_uniq:
    tmp = nucl_taxonomy_df[nucl_taxonomy_df['Family'] == family]
    seq_id = list(tmp['contig_id'].drop_duplicates())
    seq_info = tmp[['Taxid', 'contig_id', 'Accession', 'Family', 'Order', 'Genus', 'Virus_name']].drop_duplicates()
