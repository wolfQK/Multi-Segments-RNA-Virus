...
parser.add_argument('--tblastx_out_file', type=str, default=None)
...
## tblastx结果
tblastx_out_file = args.tblastx_out_file
## 相差范围 
...
tblastx_out = pd.read_csv(tblastx_out_file, sep='\t', names=names)
tblastx_out.shape
...
## 3.0 比对结果预处理
### 3.0.1 获取有效的比对结果
tblastx_use = get_useful_blast_info(blast_out=tblastx_out, related_p=80)
### 3.0.2 获取比对上的contigs-segments
contigs_segments = tblastx_use[['qseqid', 'sseqid']].drop_duplicates() # 只考虑是否有效比对，不考虑比对上了哪部分
...
### 3.0.4 整合比对信息和分类信息
contigs_segments_tblastx_taxonomy = pd.merge(tblastx_use, nucl_taxonomy_df, how='left', left_on=['qseqid', 'sseqid'], right_on=['contig_id','Accession'])

### 3.0.5 找比对上的基因组
# 以taxid来区分基因组; 还没有验证这是否可行
Taxid_list = list(contigs_segments_tblastx_taxonomy['Taxid'].drop_duplicates())

## 3.1 选比对上的基因组的每条节段的最优比对
### 3.1.1 找潜在的基因组
found_Virome = dict()
for taxid in Taxid_list:
    tmp = contigs_segments_tblastx_taxonomy[contigs_segments_tblastx_taxonomy['Taxid'] == taxid] # 该基因组的全部比对结果
