...
parser.add_argument('--blastp_out_file', type=str, default=None)
...
## blastx结果
blastp_out_file = args.blastp_out_file
## 相差范围 
...
blastp_out = pd.read_csv(blastp_out_file, sep='\t', names=names)
def prot2nucl(prot=None):
    return '_'.join(prot.split('_')[:-1])
blastp_out['qseqid'] = blastp_out['qseqid'].apply(prot2nucl)
...
def get_useful_blast_info(blast_out=None, related_p=100):
    ...
    for contig_id in blast_out['qseqid'].drop_duplicates():
        ...
        if int(tmp_df.qlen.drop_duplicates().max()) > 200: # 增加了长度筛选
...
## 3.0 比对结果预处理
### 3.0.1 获取有效的比对结果
blastp_use = get_useful_blast_info(blast_out=blastp_out, related_p=80)
### 3.0.2 获取比对上的contigs-segments
contigs_segments = blastp_use[['qseqid', 'sseqid']].drop_duplicates() # 只考虑是否有效比对，不考虑比对上了哪部分
...
### 3.0.4 整合比对信息和分类信息
contigs_segments_blastp_taxonomy = pd.merge(blastp_use, prot_taxonomy_df, how='left', left_on=['qseqid', 'sseqid'], right_on=['contig_id','Accession'])

### 3.0.5 找比对上的基因组
# 以taxid来区分基因组; 还没有验证这是否可行
Taxid_list = list(contigs_segments_blastp_taxonomy['Taxid'].drop_duplicates())

## 3.1 选比对上的基因组的每条节段的最优比对
### 3.1.1 找潜在的基因组
found_Virome = dict()
for taxid in Taxid_list:
    tmp = contigs_segments_blastp_taxonomy[contigs_segments_blastp_taxonomy['Taxid'] == taxid] # 该基因组的全部比对结果
